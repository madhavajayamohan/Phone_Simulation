...
def create_customers(log: dict[str, list[dict]]) -> list[Customer]:
    ...
    for cust in log['customers']:
        ...
        for line in cust['lines']:
            ...
            # 3) Do not change anything in the code below besides
            # uncommenting it
            # 4) Remove this
            contract = None
...
def process_event_history(log: dict[str, list[dict]],
    ...
    """
    # Finds the earliest recorded month, and creates a bill for that month
    # for each PhoneLine object a customer has depending on the PhoneLine's
    # contract
    ...
    # Loops through each call and sms in log['events']
    for event_data in log['events']:
        date = datetime.datetime.strptime(event_data['time'],
                                          "%Y-%m-%d %H:%M:%S")
        month = date.month

        # Checks if the loop has reached next month in the dataset.
        # If it has, creates a bill  for each PhoneLine object a
        # customer has depending on the PhoneLine's contract
        if month != billing_month:
            new_month(customer_list, date.month, date.year)
            billing_month = month
        # In this assignment, sms events are excluded
        if event_data['type'] != "sms":
            ...
            calle = find_customer_by_number(new_call.dst_number, customer_list)

            if caller is not None:
                caller.make_call(new_call)

            if calle is not None:
                calle.receive_call(new_call)
